...
- Phase 9: source-derived pinned Fleet contracts, runtime-oracle generators and independent reviewers for both supported Templar event families, closed disposition vocabularies, anti-curriculum-leakage rules, identity-free model projections, and separate 252-event accepted Phase 19/23 corpora are implemented. Those are combined into a content-addressed 504-event two-family runtime corpus with event-family-aware splitting. Two contamination-safe corrective curriculum cycles have now been completed. The previous production candidate reached 32/34 calibrated exact on unchanged fresh transfer with zero false-ALLOWs and 18/20 exact on the first Psalmbox-held hidden adversarial bank with zero false-ALLOWs and zero false-DENYs. A real Fleet integration proof is also complete: Agoge can verify and answer bound `fleet.templar-evaluation-request.v1` documents, and a persistent mode-0600 Unix-socket runtime reduces warmed Fleet-to-model-to-Fleet latency to ~47 ms while preserving authority-free verdicts and Fleet fail-closed behavior.
- Phase 10: the first Templar adversarial-school vertical slice is implemented. Deterministic mutation candidates use a closed non-training schema; sealed Exam prompts cannot become mutation sources; generated failures are clustered without copying raw held-out bodies into corrective requests; and corrective examples must be regenerated through pinned Fleet semantics, independently reviewed, and promoted before entering a versioned corpus. A 1,008-case development adversarial school exposed one benign-context-dilution weakness in the previous adapter. Two full-from-base 150-step corrective candidates were rejected because untouched fresh-transfer safety regressed. A new resume-from-proven-adapter strategy then found the bounded repair: five optimizer steps at `1e-5`, starting from adapter `sha256:b5811a90fe6ece26a05d48d91684e466b3b4937af61705ee2cd2e6faf4df08f8`, produced adapter `sha256:d4ba373f9dcb2f9f997783001faa4b691fa58f3c063c927faad2183da613358b`. That candidate reaches 1,008/1,008 exact on the raw generated adversarial school, 34/34 exact on the unchanged reviewable fresh-transfer bank, and 18/20 exact on the unchanged external-hidden adversarial bank, with zero false-ALLOWs and zero false-DENYs on all three. Templar is still not production-graduated; broader release/security and production Fleet regression gates remain open.
