...
- Phase 3: deterministic prototype split plus content-addressed, training-forbidden Exam banks are implemented. Templar now has a 34-case reviewable fresh-transfer bank with exact contamination checks against accepted corpora; the external-hidden adversarial bank remains open.
...
- Phase 6: strict local and API Runtime Exams, base/adapter comparison, exact-disposition scoring, false-ALLOW/false-DENY metrics, contract validity, local/API latency measurements, and sealed-bank sequence-classifier/API evaluators are implemented. The first corrective Templar Askesis raised the unchanged fresh-transfer bank from 23/34 exact with seven false-ALLOWs to 32/34 exact with one false-ALLOW; adversarial and calibration suites remain open.
...
- Phase 9: source-derived pinned Fleet contracts, runtime-oracle generators and independent reviewers for both supported Templar event families, closed disposition vocabularies, anti-curriculum-leakage rules, identity-free model projections, and separate 252-event accepted Phase 19/23 corpora are implemented. Those are combined into a content-addressed 504-event two-family runtime corpus with event-family-aware splitting. Fresh-transfer failure diagnosis has now driven one contamination-safe corrective curriculum cycle: all regenerated Phase 23 candidates passed independent admission 252/252, the corrected 504-event corpus reaches 46/46 exact on both internal validation and test, and the unchanged 34-case fresh-transfer bank reaches 32/34 exact. One hidden-instruction REVIEW still becomes a dangerous ALLOW, so Templar remains non-graduated; calibration, immutable hidden adversarial banks, and production Fleet integration remain open.
